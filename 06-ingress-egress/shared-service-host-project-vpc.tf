...
# Subnets
module "shared_vpc_subnet_1" {
  source                  = "github.com/john-hurringjr/test-modules/networking/subnet/gke"
  project_id              = module.shared_vpc_host_project.project_id
  network_self_link       = google_compute_network.shared_project_vpc.self_link
  network_name            = google_compute_network.shared_project_vpc.name
  region                  = var.region
...
module "shared_vpc_restricted_apis_routing" {
...
module "shared_vpc_prod_firewall_allow_iap_all" {
...
resource "google_compute_route" "default_shared_route" {
  project           = module.shared_vpc_host_project.project_id
  dest_range        = "0.0.0.0/0"
  name              = "shared-default"
  network           = google_compute_network.shared_project_vpc.name
