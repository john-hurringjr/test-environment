...
resource "google_compute_network" "vpc_sc_isolated_vpc_iap_testing" {
  project                         = module.vpc_sc_test_project_dev_1.project_id
  name                            = "isolated-vpc"
  routing_mode                    = "GLOBAL"
...
module "vpc_sc_isolated_vpc_iap_testing_region_1_subnet" {
  source                = "github.com/john-hurringjr/test-modules/networking/subnet/generic"
  project_id            = module.vpc_sc_test_project_dev_1.project_id
  network_self_link     = google_compute_network.vpc_sc_isolated_vpc_iap_testing.self_link
  network_name          = google_compute_network.vpc_sc_isolated_vpc_iap_testing.name
  region                = var.region_1
...
module "vpc_sc_isolated_vpc_iap_testing_region_2_subnet" {
  source                = "github.com/john-hurringjr/test-modules/networking/subnet/generic"
  project_id            = module.vpc_sc_test_project_dev_1.project_id
  network_self_link     = google_compute_network.vpc_sc_isolated_vpc_iap_testing.self_link
  network_name          = google_compute_network.vpc_sc_isolated_vpc_iap_testing.name
...
module "vpc_sc_isolated_vpc_firewall_allow_iap_all" {
